...
def smoothed_pgd_loop(
    ...
    Based on "Towards Understanding and Improving the Transferability of
    Adversarial Examples in Deep Neural Networks" (Wu & Zhu, 2020)
    ...
    for _ in range(nb_iter):
        ...
        for i in range(smoothing_m):
            ...
            batch.sig = wav_init + delta + noise, wav_lens
            predictions = asr_brain.compute_forward(batch, rs.Stage.ATTACK)
            loss = asr_brain.compute_objectives(
                predictions, batch, rs.Stage.ATTACK)
            if minimize:
            ...
            # Accumulate gradient
            smoothed_grad += delta.grad.data / smoothing_m
            
            # Clear gradient for next iteration
            delta.grad.data.zero_()
        
        # Update delta using smoothed gradient
        if order == np.inf:
            grad_sign = smoothed_grad.sign()
            delta.data = delta.data + eps_iter * grad_sign
            delta.data = linf_clamp(delta.data, eps)
            delta.data = (
                torch.clamp(wav_init.data + delta.data, clip_min, clip_max)
            ...
            grad = l2_clamp_or_normalize(smoothed_grad)
            delta.data = delta.data + eps_iter * grad
            delta.data = (
                torch.clamp(wav_init.data + delta.data, clip_min, clip_max)
                - wav_init.data
...
class ASRPGDAttack(Attacker):
    ...
    use_smoothed_gradient: bool
       whether to use smoothed gradient attack (default: False)
    smoothing_m: int
       number of samples for gradient smoothing (default: 20)
    smoothing_sigma: float
       standard deviation of Gaussian noise for smoothing (default: 0.01)
    ...
    def perturb(self, batch):
        ...
        # Choose between smoothed and regular PGD
        if self.use_smoothed_gradient:
            wav_adv = smoothed_pgd_loop(
                batch,
                self.asr_brain,
    ...
    def perturb_and_log_return_perturbation(self, batch, mode):
        ...
        if self.use_smoothed_gradient:
            if mode == 'train':
                wav_adv, perturbation = smoothed_pgd_loop_with_return_delta(
                    batch,
                    self.asr_brain,
                    nb_iter=self.nb_iter,
                    eps=self.eps,
...
class ASRL2PGDAttack(ASRPGDAttack):
    ...
    def __init__(
        ...
        super(ASRL2PGDAttack, self).__init__(
            ...
            order=order,
            use_smoothed_gradient=use_smoothed_gradient,
